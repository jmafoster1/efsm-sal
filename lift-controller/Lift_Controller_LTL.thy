...
lemma possible_steps_0: "possible_steps lift_lit 0 r STR ''continit'' [] = {|(9, continit)|}"
  apply (simp add: possible_steps_singleton set_eq_iff lift_lit_def)
  apply clarify
  apply (case_tac "a=0")
  by (auto simp add: continit_def)

lemma ltl_step_0: "ltl_step lift_lit (Some 0) r (STR ''continit'', []) = (Some 9, [], r(1 $:= Some (Str ''true'')))"
  by (simp add: possible_steps_0 continit_def apply_updates_def)

lemma ltl_step_0_invalid: "e \<noteq> (STR ''continit'', []) \<Longrightarrow> ltl_step lift_lit (Some 0) r e = (None, [], r)"
  apply (cases e)
  apply (simp del: ltl_step.simps)
  apply (rule ltl_step_none)
  apply (simp add: possible_steps_empty lift_lit_def continit_def can_take)
  by auto
...
lemma no_motor_stop: "s \<notin> {1, 2, 3, 4} \<Longrightarrow> possible_steps lift_lit s r STR ''motorstop'' i = {||}"
  apply (simp add: possible_steps_empty lift_lit_def)
  by (simp add: transitions)

lemma no_open_door: "s \<notin> {5, 6, 7, 8} \<Longrightarrow> possible_steps lift_lit s r STR ''opendoor'' i = {||}"
  apply (simp add: possible_steps_empty lift_lit_def)
  by (simp add: transitions)

lemma possible_steps_9_invalid: "(l, i) \<noteq> (STR ''return'', []) \<Longrightarrow> possible_steps lift_lit 9 r l i = {||}"
  apply (simp add: possible_steps_empty lift_lit_def)
...
lemma possible_steps_9_invalid_r4:
  ...
  shows "possible_steps lift_lit 9 r l i = {||}"
  apply (simp add: possible_steps_empty lift_lit_def)
...
"possible_steps lift_lit s r (fst (shd t)) (snd (shd t)) = {||} \<Longrightarrow>
fst (shd t) = STR ''opendoor'' \<Longrightarrow>
 \<not> nxt (output_eq [Some (Str ''true''), Some (Num n)]) (make_full_observation lift_lit (Some s) r p t)"
    apply (rule not_nxt)
...
possible_steps lift_lit 9 r (STR ''return'') [] = {|(nat n, return n)|}"
  apply (simp add: possible_steps_singleton lift_lit_def set_eq_iff)
  apply clarsimp
  apply (case_tac "a=9")
...
ltl_step lift_lit (Some n) r e = (None, [], r)"
  apply (cases e)
  apply (simp del: ltl_step.simps)
  apply (rule ltl_step_none)
  apply (simp add: possible_steps_empty lift_lit_def)
...
lemma up_4: "fst e = STR ''up'' \<Longrightarrow> ltl_step lift_lit (Some 4) r e = (None, [], r)"
  apply (cases e)
  apply (simp del: ltl_step.simps)
  apply (rule ltl_step_none)
  apply (simp add: possible_steps_empty lift_lit_def)
...
lemma ltl_step_lift_lit_deterministic:
  "ltl_step lift_lit (Some n) r e = (Some aa, b, c) \<Longrightarrow>
   \<exists>t. possible_steps lift_lit n r (fst e) (snd e) = {|(aa, t)|}"
  apply (simp add: lift_lit)
...
lemma ltl_step_floor:
  "fst e \<noteq> STR ''motorstop'' \<Longrightarrow>
   ...
   aa \<in> {1, 2, 3, 4}"
  apply (cases e)
  apply (insert ltl_step_lift_lit_deterministic[of n r e aa b c])
  apply simp
  apply (erule exE)
  apply simp
  ...
  apply (erule_tac x=aa in allE)
    ...
    apply (case_tac "((n, aa), t) \<in> fset lift_lit")
     defer apply simp
    apply simp
    apply (erule conjE)+
    apply (simp add: lift_lit_def)
    apply (case_tac "n=1")
...
lemma must_stop_to_open_aux2:
    assumes "\<exists> n r p t. j = (make_full_observation lift_lit (Some n) r p t) \<and> (n = 1 \<or> n = 2 \<or> n = 3 \<or> n = 4)"
    shows "
  ...
  apply standard apply auto[1]
  apply standard apply auto[1]
  apply (case_tac "ltl_step lift_lit (Some n) r (shd t)")
  apply (case_tac a)
   apply simp
   apply (rule disjI2)
...
lemma must_stop_to_open_aux3:
  ...
  apply (case_tac "ltl_step lift_lit (Some na) r (shd t)")
  apply (case_tac a)
   apply simp
   apply (rule disjI2)
   apply (rule alw_implies_until)
...
lemma must_stop_to_open_aux1:
    assumes "\<exists> s r p t. j = (make_full_observation lift_lit (Some 9) r p t)"
  ...
  apply (case_tac "\<exists>n. r $ 4 = Some (Num n) \<and> n \<in> {int 1, int 2, int 3, int 4}")
   defer apply auto[1]
  apply (erule exE)
  apply (case_tac "possible_steps lift_lit 9 r (STR ''return'') [] = {|(nat n, return n)|}")
...
lemma possible_steps_opendoor_invalid_state:
  "sa \<noteq> 5 \<Longrightarrow>
   ...
   possible_steps lift_lit sa ra STR ''opendoor'' b = {||}"
  apply (simp add: possible_steps_empty lift_lit_def)
  by (simp add: transitions)
...
lemma must_stop_to_open_aux1a:
    assumes "\<exists> r p t. j = (make_full_observation lift_lit (Some 9) r p t)"
  ...
  apply (case_tac "\<exists>n. r $ 4 = Some (Num n) \<and> n \<in> {int 1, int 2, int 3, int 4}")
   defer apply auto[1]
  apply (erule exE)
  apply (case_tac "possible_steps lift_lit 9 r (STR ''return'') [] = {|(nat na, return na)|}")
  defer apply (simp add: return)
  apply simp
  apply (rule disjI2)
  subgoal for x r p t a b na
    using must_stop_to_open_aux3[of "(make_full_observation lift_lit (Some (nat na)) (apply_updates [] (join_ir [] r) r) [] (stl t))" n]
    apply (simp add: nxt.simps)
    by fastforce
  done
...
lemma must_stop_to_open:
  shows "((not (label_eq ''opendoor'' aand nxt (output_eq [Some (Str ''true''), Some (Num n)]))) until (label_eq ''motorstop'')) (make_full_observation lift_lit (Some 0) r p t)"
  ...
  using must_stop_to_open_aux1a[of "(make_full_observation lift_lit (Some 9) (r(1 $:= Some (EFSM.Str ''true''))) [] (stl t))"]
  by fastforce

lemma ltl_step_9_invalid: "\<not>(\<exists>n. r $ 4 = Some (Num n) \<and> n \<in> {1, 2, 3, 4}) \<Longrightarrow> ltl_step lift_lit (Some 9) r e = (None, [], r)"
  apply (cases e)
  apply (simp del: ltl_step.simps)
  apply (rule ltl_step_none)
  apply (simp add: possible_steps_empty lift_lit_def)
...
"(ffilter (\<lambda>((origin, dest), t). Label t = STR ''motorstop'') lift_lit) =
{|((f4, o4), motorstop4),
  ((f3, o3), motorstop3),
  ((f2, o2), motorstop2),
  ((f1, o1), motorstop1)
|}"
  apply (simp add: lift_lit_def Abs_ffilter set_eq_iff)
  apply clarify
   apply (simp add: transitions)
  apply (case_tac "a=1")
   apply (simp add: transitions)
...
"r $ 1 = Some (EFSM.Str ''true'') \<Longrightarrow>
 n \<in> {1, 2, 3, 4} \<Longrightarrow>
 ltl_step lift_lit (Some n) r (STR ''motorstop'', [EFSM.Str ''true'', EFSM.Str ''true'']) = (Some (n + 4), [Some (Num 0), Some (Num n), Some (Str ''true'')], r)"
...
"r $ 1 \<noteq> Some (EFSM.Str ''true'') \<Longrightarrow>
 n \<in> {1, 2, 3, 4} \<Longrightarrow>
 ltl_step lift_lit (Some n) r (STR ''motorstop'', [EFSM.Str ''true'', EFSM.Str ''true'']) = (None, [], r)"
  apply (rule ltl_step_none)
...
lemma alw_must_stop_to_open_end:
  "alw ((\<lambda>xs. label (shd xs) = STR ''opendoor'' \<longrightarrow> \<not> nxt (output_eq [Some (Str ''true''), Some (Num n)]) xs) until
        ...
        (make_full_observation lift_lit None r [] t)"
...
lemma alw_must_stop_to_open:
    assumes "\<exists>s r p t. j = (make_full_observation lift_lit (Some s) r p t) \<and> s \<notin> {5, 6, 7, 8}"
...
lemma bad_step:
  assumes "ltl_step lift_lit (Some s) r (shd t) = (None, [], r)"
shows "((ev (nxt ((label_eq ''opendoor '') aand (nxt (output_eq [Some(Str ''true''), Some n]))))) impl
      ...
      (make_full_observation lift_lit (Some s) r p t)"
proof-
  have aux: "\<not>ev (nxt (\<lambda>xs. label_eq ''opendoor '' xs \<and> nxt (output_eq [Some (EFSM.Str ''true''), Some n]) xs))
     (\<lparr>statename = Some s, datastate = r, action = shd t, output = p\<rparr> ## make_full_observation lift_lit None r [] (stl t))"
  ...
  show ?thesis
    using assms
    by (simp add: make_full_observation.ctr[of lift_lit "Some s"] aux)
qed
...
"e \<noteq> (STR ''return'', []) \<Longrightarrow>
  ltl_step lift_lit (Some 9) r e = (None, [], r)"
  apply (case_tac e, simp)
  apply (rule ltl_step_none)
  apply (simp add: possible_steps_empty lift_lit_def)
  by (simp add: transitions can_take)

lemma opendoor_9: "ltl_step lift_lit (Some 9) r (STR ''opendoor'', b) = (None, [], r)"
  apply (rule ltl_step_none)
  by (simp add: possible_steps_empty lift_lit_def transitions)

lemma opendoor_invalid: "s \<notin> {5, 6, 7, 8} \<Longrightarrow> ltl_step lift_lit (Some s) r (STR ''opendoor'', b) = (None, [], r)"
  apply (rule ltl_step_none)
  by (simp add: possible_steps_empty lift_lit_def transitions)

lemma opendoor_invalid_nat: "s \<notin> {5, 6, 7, 8} \<Longrightarrow> ltl_step lift_lit (Some (nat s)) r (STR ''opendoor'', b) = (None, [], r)"
  apply (rule ltl_step_none)
  apply (simp add: possible_steps_empty lift_lit_def transitions)
  by auto
...
lemma stop_at_None: "alw (\<lambda>xs. label (shd (stl xs)) = a \<longrightarrow> output (shd (stl (stl xs))) \<noteq> [Some (EFSM.Str ''true''), Some n])
        (make_full_observation lift_lit None r [] s)"
  apply (rule alw_mono[of "alw (output_eq [])"])
   apply (simp add: no_output_none_if_empty)
  by fastforce
...
lemma prem_ignore_fst:
 ...
 ev (\<lambda>a. label (shd (stl a)) = STR ''opendoor'' \<and> output_eq [Some (EFSM.Str ''true''), Some n] (stl (stl a)))
        (ss ## make_full_observation lift_lit (Some s') r' p' ((a, b) ## x2)) =
 ev (\<lambda>a. label (shd (stl a)) = STR ''opendoor'' \<and> output_eq [Some (EFSM.Str ''true''), Some n] (stl (stl a)))
        (make_full_observation lift_lit (Some s') r' p' ((a, b) ## x2))"
  apply standard
...
lemma prem_stop_at_none_scons: "\<not> ev (\<lambda>a. label (shd (stl a)) = STR ''opendoor'' \<and> output_eq [Some (EFSM.Str ''true''), Some n] (stl (stl a)))
        (ss ## make_full_observation lift_lit None r [] t)"
  apply (simp add: not_ev_iff)
  apply (coinduction)
...
lemma label_up:
"ffilter (\<lambda>((origin, dest), t). Label t = STR ''up'') lift_lit = {|
    ((f3, f4), up34stop),
    ((f2, f3), up23stop),
    ((f2, f3), up23),
    ((f1, f2), up12stop),
    ((f1, f2), up12)
|}"
  apply (simp add: Abs_ffilter lift_lit_def set_eq_iff)
  apply clarify
  apply (case_tac "a=1")
...
lemma not_motorstop_step_state:
  "s \<in> {1, 2, 3, 4} \<Longrightarrow>
   ...
   ltl_step lift_lit (Some s) r e = (Some s', p, r') \<Longrightarrow>
   s' \<in> {1, 2, 3, 4}"
  using ltl_step_lift_lit_deterministic[of s r e s' p r']
  apply simp
  apply (erule exE)
  apply (simp add: possible_steps_alt3 Abs_ffilter lift_lit_def set_eq_iff)
...
lemma ltl_step_state_none: "(ltl_step lift_lit (Some s) r e = (None, b, c)) \<Longrightarrow> b = [] \<and> c = r"
  apply (cases e)
  apply (simp add: ltl_step.simps Let_def)
  apply (case_tac "possible_steps lift_lit s r a ba = {||}")
   apply simp
  apply (case_tac "SOME x. x |\<in>| possible_steps lift_lit s r a ba")
  by simp

lemma opendoor_moving:
  "s \<in> {1, 2, 3, 4} \<Longrightarrow> ltl_step lift_lit (Some s) r (STR ''opendoor'', ba) = (None, [], r)"
  apply (rule ltl_step_none)
  apply (simp add: possible_steps_empty lift_lit_def)
  apply (simp add: transitions)
  by auto

lemma label_opendoor: "ffilter (\<lambda>((origin, dest), t). Label t = STR ''opendoor'') lift_lit =
       {|((5, 5), opendoor1), ((6, 6), opendoor2), ((7, 7), opendoor3), ((8, 8), opendoor4)|}"
  apply (simp add: Abs_ffilter set_eq_iff lift_lit_def)
  apply clarify
  apply (case_tac "a=5")
   apply (simp add: transitions)
   apply auto[1]
...
lemma ltl_step_opendoor:
  "s \<in> {5, 6, 7, 8} \<Longrightarrow> ltl_step lift_lit (Some s) r (STR ''opendoor'', [EFSM.Str ''true'']) = (Some s, [Some (Str ''true''), Some (Num (s-4))], r)"
...
lemma possible_steps_lift_lit_fset:
  "possible_steps lift_lit s r STR ''opendoor'' b \<noteq> {||} \<Longrightarrow>
   \<exists>s' t. possible_steps lift_lit s r STR ''opendoor'' b = {|(s', t)|}"
  apply (simp add: lift_lit)
...
lemma output_opendoors:
  ...
  fst (snd (ltl_step lift_lit (Some s) r e)) \<noteq> [Some (EFSM.Str ''true''), Some n]"
  apply (cases e)
  apply (simp add: ltl_step.simps)
  apply (case_tac "possible_steps lift_lit s r STR ''opendoor'' b = {||}")
   apply simp
  apply simp
  apply (case_tac "SOME x. x |\<in>| possible_steps lift_lit s r STR ''opendoor'' b")
  apply simp
  subgoal for l i s' t
    apply (insert possible_steps_lift_lit_fset[of s r i])
    apply simp
    apply (erule exE)+
...
lemma ltl_step_ocd_invalid:
  "e \<noteq> (STR ''opendoor'', [EFSM.Str ''true'']) \<Longrightarrow>
   ...
   ltl_step lift_lit (Some s) r e = (None, [], r)"
  apply (cases e, simp)
  apply (rule ltl_step_none)
  apply (simp add: possible_steps_empty lift_lit_def)
  apply (erule disjE)
...
lemma ltl_step_startsearch:
  "s \<in> {5, 6, 7, 8} \<Longrightarrow>
  ltl_step lift_lit (Some s) r (STR ''startsearch'', [EFSM.Str ''true'', EFSM.Str ''false'']) = (Some 9, [], r)"
  apply (rule ltl_step_some[of _ _ _ _ _ 9 startsearch])
    defer apply simp
   apply (simp add: apply_updates_def)
  apply (simp add: lift_lit_def possible_steps_singleton set_eq_iff)
  apply clarify
...
lemma possible_steps_lift_lit: "possible_steps lift_lit s r a ba \<noteq> {||} \<Longrightarrow>
       \<exists>aa t. possible_steps lift_lit s r a ba = {|(aa, t)|}"
  apply (simp add: lift_lit)
  using deterministic_alt deterministic_lift possible_steps_alt by blast

lemma ltl_step_Some_lift_lit: "(ltl_step lift_lit (Some s) r e = (Some aa, b, c)) =
       (\<exists>t. possible_steps lift_lit s r (fst e) (snd e) = {|(aa, t)|} \<and> evaluate_outputs t (snd e) r = b \<and> evaluate_updates t (snd e) r = c)"
  apply standard
  ...
  using possible_steps_lift_lit by fastforce

lemma ltl_step_floor_output:
  "s \<in> {1, 2, 3, 4} \<Longrightarrow> fst (snd (ltl_step lift_lit (Some s) r e)) \<noteq> [Some (EFSM.Str ''true''), Some n]"
  apply (case_tac "ltl_step lift_lit (Some s) r e")
  apply simp
  apply (case_tac a)
   apply (metis list.simps(3) ltl_step_state_none)
  apply (simp add: ltl_step_Some_lift_lit)
  apply (erule_tac exE)
  apply (erule conjE)+
  apply (simp add: possible_steps_singleton set_eq_iff)
  apply (erule_tac x=s in allE)
  ...
  subgoal for a b c aa t
    ...
    apply (simp add: lift_lit_def apply_outputs_def Str_def)
    apply (erule disjE)
     apply (simp add: transitions)
...
lemma invalid_state:
  ...
  by (simp add: possible_steps_empty lift_lit_def)

text_raw\<open>\snip{alwMustStopToOpenAux}{1}{2}{%\<close>
lemma alw_must_stop_to_open_aux:
  assumes "\<exists>s r p t. j= make_full_observation lift_lit (Some s) r p t"
...
proof-
  {assume "(ev (nxt ((label_eq ''opendoor'') aand (nxt (output_eq [Some(Str ''true''), Some n]))))) j \<and>
           (\<exists>s r p t. j= make_full_observation lift_lit (Some s) r p t)"
   hence "((not (nxt (label_eq ''opendoor'' aand (nxt (output_eq [Some(Str ''true''), Some n]))))) until(((label_eq ''motorstop'') or (nxt (output_eq [Some(Str ''true''), Some n]))))) j"
     ...
     apply (case_tac "s=0")
      apply (case_tac "shd t = (STR ''continit'', [])")
       apply (simp add: ltl_step_0 make_full_observation.ctr[of lift_lit "Some 0"])
       apply (case_tac "stl t", case_tac x1)
       apply (simp add: opendoor_invalid ev_Stream)
       apply (erule disjE)
        apply clarify
        apply (simp add: make_full_observation.ctr[of lift_lit "Some 9"] opendoor_invalid)
       apply auto[1]
      apply (simp add: make_full_observation.ctr[of lift_lit "Some 0"] ltl_step_0_invalid ev_Stream prem_stop_at_none)
      apply (simp add: ltl_step.simps)

     apply (case_tac "s \<in> {5, 6, 7, 8}")
     subgoal for x s r p t
       apply (case_tac "shd t = (STR ''opendoor'', [Str ''true''])")
        apply (simp add: ltl_step_opendoor ev_Stream make_full_observation.ctr[of lift_lit "Some s" r p t])
       ...
       apply (metis (no_types, hide_lams) fst_conv list.simps(3) opendoor_9 prod.collapse snd_conv)
        apply (rule disjI1)
        apply standard
         apply (simp add: ltl_step_startsearch ev_Stream make_full_observation.ctr[of lift_lit "Some s" r p t])
         apply (metis (no_types, lifting) Pair_inject list.simps(3) opendoor_9 prod.collapse)
        apply fastforce
       apply (simp add: ev_Stream make_full_observation.ctr[of lift_lit "Some s" r p t] ltl_step_ocd_invalid)
       apply (erule disjE)
        apply (simp add: ltl_step.simps)
     ...
     subgoal for x s r p t
       ...
       apply standard
        apply simp
        apply (rule impI)
        apply (case_tac "ltl_step lift_lit (Some s) r (shd t)", case_tac a)
         apply (simp add: ltl_step.simps)
        apply simp
       ...
       apply simp
        apply (case_tac "ltl_step lift_lit (Some s) r (shd t)", case_tac a)
         apply (simp add: ev_Stream make_full_observation.ctr[of lift_lit "Some s" r p t] ltl_step.simps)
         apply (simp add: ev_Stream make_full_observation.ctr[of lift_lit "Some s" r p t])
     ...
     apply (case_tac "s=9")
      ...
      apply standard
       ...
       apply (case_tac "\<exists>n. r $ 4 = Some (Num n) \<and> n \<in> {1, 2, 3, 4}")
        apply (case_tac "shd t = (STR ''return'', [])")
         apply (erule exE)
         apply (simp add: ev_Stream make_full_observation.ctr[of lift_lit "Some 9"] return ltl_step.simps apply_updates_def)
         apply (metis (no_types, lifting) insertCI ltl_step_floor_output nat_numeral numeral_Bit0 numeral_eq_one_iff)

        apply (case_tac "shd t")
        apply (simp add: ev_Stream make_full_observation.ctr[of lift_lit "Some 9"] possible_steps_9_invalid ltl_step.simps)

       apply (case_tac "shd t", simp add: ev_Stream make_full_observation.ctr[of lift_lit "Some 9"] ltl_step_9_invalid)
       apply (simp add: ltl_step.simps(1))

      apply (case_tac "ltl_step lift_lit (Some s) r (shd t)", case_tac a)
       apply (simp add: ev_Stream make_full_observation.ctr[of lift_lit "Some 9"] ltl_step.simps)
     using prem_stop_at_none ltl_step_state_none apply blast
      apply fastforce
...
lemma alw_must_stop_to_open:
  "alw ((ev (nxt ((label_eq ''opendoor'') aand
       ...
       ((not (nxt ((label_eq ''opendoor'') aand
        (nxt (output_eq [Some (Str ''true''), Some  n]))))) until
       (((label_eq ''motorstop'') or (nxt (output_eq [Some (Str ''true''), Some  n])))))) (watch lift_lit i)"
text_raw\<open>}%endsnip\<close>
...
lemma LTL_must_stop_lift_to_open_door: "alw ((ev (nxt ((label_eq ''opendoor'') aand (nxt (output_eq [Some(Str ''true''), Some n]))))) impl
      ((not (nxt ((label_eq ''opendoor'') aand (nxt (output_eq [Some(Str ''true''), Some n]))))) suntil
      (((label_eq ''motorstop'') or (nxt (output_eq [Some(Str ''true''), Some n])))))) (watch lift_lit t)"
