...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=4.7cm,semithick, initial text={}, state/.style={circle, draw, minimum size=0.5cm}]
  ...
  \draw (s0) edge node{$\mathit{login}:1[i_0 = "free"]$} (s1);
  \draw (s0) edge node{$\mathit{login}:1[i_0 = "paid"]$} (s1);
  \draw (s1) edge node{$\mathit{view}:3[i_0 = "friendID", i_0 = "name", i_0 = "HM8p"]$} (s2);
  \draw (s1) edge node{$\mathit{view}:3[i_0 = "otherID", i_0 = "OUT_OF_NETWORK", i_0 = "MNn5"]$} (s4);
  \draw (s1) edge node{$\mathit{view}:3[i_0 = "otherID", i_0 = "name", i_0 = "4zoF"]$} (s6);
  \draw (s1) edge node{$\mathit{view}:3[i_0 = "otherID", i_0 = "name", i_0 = "MNn5"]$} (s6);
