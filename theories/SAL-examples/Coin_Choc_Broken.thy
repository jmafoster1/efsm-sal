...
lemma "alw (\<lambda>s. state_eq None (stl s)) (make_full_observation drinks None Map.empty t)"
...
lemma less_than_zero_not_nxt_2: "n \<le> 0 \<Longrightarrow> \<not>statename (shd (stl (make_full_observation drinks (Some 1) <1 := Num n> t))) = Some 2"
...
proof(coinduction)
  ...
  proof(coinduction)
    case alw
    then show ?case
      apply (simp add: vend_insufficient apply_updates_init)
      apply (rule disjI2)
      using alw_not_some
      by simp
  qed
