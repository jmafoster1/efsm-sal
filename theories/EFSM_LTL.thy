...
definition EventEq :: "(string \<times> inputs) \<Rightarrow> state stream \<Rightarrow> bool" where
  "EventEq e = label_eq (fst e) aand input_eq (snd e)"

definition output_eq :: "value option list \<Rightarrow> state stream \<Rightarrow> bool" where
  "output_eq v s \<equiv> output (shd s) = v"

fun check_inx :: "ior \<Rightarrow> nat \<Rightarrow> (value option \<Rightarrow> value option \<Rightarrow> trilean) \<Rightarrow> value option \<Rightarrow> state stream \<Rightarrow> bool" where
  "check_inx ior.ip n f v s = (f (Some (nth (inputs (shd s)) (n-1))) v = trilean.true)" |
  "check_inx ior.op n f v s = (f (nth (output (shd s)) n) v = trilean.true)" |
  "check_inx ior.rg n f v s = (f (datastate (shd s) $ n) v = trilean.true)"
